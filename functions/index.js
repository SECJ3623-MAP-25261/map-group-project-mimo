...
// ✅ Cloud Function for generating rental insights
// Note: App Check enforcement should be disabled in Firebase Console
exports.generateRentalInsights = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const totalSpent = bookings.reduce((sum, b) => sum + (b.finalFee || 0), 0);
    const moneySaved = bookings.reduce((sum, b) => {
      return sum + Math.max(0, (b.itemPrice || 0) - (b.finalFee || 0));
    }, 0);

    // Calculate return score (on-time returns)
    ...
    const avgBuy = totalRentals 
      ? bookings.reduce((s, b) => s + (b.itemPrice || 0), 0) / totalRentals 
      : 0;
    const percentSaved = avgBuy > 0 
      ? Math.round(((avgBuy - avgRental) / avgBuy) * 100) 
      : 0;
    ...
    console.error('Error generating rental insights:', error);
